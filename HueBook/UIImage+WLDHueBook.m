...
-(UIImage *) blendWithForground:(UIImage *)forgroundImage inBlendMode:(CGBlendMode)mode{
    ...
    CGContextDrawImage(context, CGRectMake(0, 0, width, height), forgroundImage.CGImage);
...
-(UIImage *) overlayWithForground:(UIImage *)forgroundImage inSize:(CGSize)size{
    ...
    [forgroundImage drawInRect:CGRectMake(0, 0, size.width, size.height)];
    UIImage *ouput = UIGraphicsGetImageFromCurrentImageContext();
...
- (UIImage *) overlayLineMap:(UIImage *)lineMap{
    ...
    CGContextDrawImage(context, CGRectMake(0.0, 0.0, width, height), lineMap.CGImage);
...
- (UIImage *) floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor andTolerance:(int)tolerance useAntiAlias:(BOOL)antiAlias
{
    @try
    {
        ...
        */
        ...
        unsigned char *outputImageData = malloc(height * width * 4);
        ...
        */
        ...
        while ([points popFront:&x andY:&y] != INVALID_NODE_CONTENT)
        {
            ...
            while (y < height && compareColor(ocolor, color, tolerance) && ncolor != color)
            {
                ...
                outputImageData[byteIndex + 0] = 0;
                outputImageData[byteIndex + 1] = 0;
                outputImageData[byteIndex + 2] = 0;
                outputImageData[byteIndex + 3] = 255;
...
//                        
        ...
        CGContextRef context2 = CGBitmapContextCreate(outputImageData,
                                                     width,
                                                     height,
                                                     bitsPerComponent,
                                                     bytesPerRow,
